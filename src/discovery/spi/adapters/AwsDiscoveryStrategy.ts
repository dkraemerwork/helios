/**
 * AwsDiscoveryStrategy — wraps AwsDiscoveryProvider as a DiscoveryStrategy.
 *
 * Accepted config properties (mirrors DiscoveryConfig.properties):
 *   region   — AWS region (default: 'us-east-1')
 *   port     — Helios member port (default: 5701)
 *   usePublicIp — 'true' to discover public IPs (default: 'false')
 */

import { AwsDiscoveryProvider } from '@zenystx/helios-core/discovery/HeliosDiscovery';
import type { DiscoveredNode, DiscoveryStrategy, DiscoveryStrategyFactory } from '@zenystx/helios-core/discovery/spi/DiscoverySPI';

export class AwsDiscoveryStrategy implements DiscoveryStrategy {
  private readonly _provider = new AwsDiscoveryProvider();
  private readonly _config: Record<string, string>;

  constructor(config: Record<string, string>) {
    this._config = config;
  }

  async start(): Promise<void> {
    // No persistent connection required; fetch() is stateless.
  }

  async discoverNodes(): Promise<DiscoveredNode[]> {
    const members = await this._provider.discover({
      provider: 'aws',
      properties: this._config,
    });
    return members.map(m => ({
      address: { host: m.host, port: m.port },
      properties: new Map(Object.entries(this._config)),
    }));
  }

  async destroy(): Promise<void> {
    // No resources to release.
  }

  getPartitionGroupStrategy(): string {
    return 'ZONE_AWARE';
  }
}

export class AwsDiscoveryStrategyFactory implements DiscoveryStrategyFactory {
  getDiscoveryStrategyType(): string {
    return 'aws';
  }

  newDiscoveryStrategy(config: Record<string, unknown>): DiscoveryStrategy {
    const stringConfig: Record<string, string> = {};
    for (const [k, v] of Object.entries(config)) {
      stringConfig[k] = String(v);
    }
    return new AwsDiscoveryStrategy(stringConfig);
  }
}
