...
export class DataHandler {
    ...
    private async _handleMap(req: Request, path: string, method: string): Promise<Response> {
    ...
    private async _handleQueue(req: Request, path: string, method: string): Promise<Response> {
        ...
        if (slash === -1) {
            ...
            if (method === 'POST') {
                ...
                if (queue === null) return this._notFound(`Queue '${queueName}' not found.`);
                const body = await this._parseJson(req);
                const accepted = queue.offer(body);
                if (!accepted) {
                    return new Response(
        ...
        if (segment === 'size') {
            return new Response(JSON.stringify({ size: queue.size() }), { status: 200, headers: JSON_HEADERS });
        }
    ...
    private _notFound(message: string): Response {
        return new Response(
            JSON.stringify({ status: 404, message }),
