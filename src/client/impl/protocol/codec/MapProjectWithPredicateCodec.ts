/**
 * Port of {@code com.hazelcast.client.impl.protocol.codec.MapProjectWithPredicateCodec}.
 *
 * Applies the projection logic on map entries filtered with the Predicate and returns the result.
 *
 * Request:  name (string), projection (Data), predicate (Data)
...
export class MapProjectWithPredicateCodec {
    ...
    static readonly RESPONSE_MESSAGE_TYPE = 0x013C01;

    private constructor() {}

    static decodeRequest(msg: ClientMessage): { name: string; projection: Data; predicate: Data } {
        const iter = msg.forwardFrameIterator();
        iter.next(); // initial frame (empty)
        const name = StringCodec.decode(iter);
        const projection = DataCodec.decode(iter);
        const predicate = DataCodec.decode(iter);
        return { name, projection, predicate };
    }
    ...
    static encodeResponse(items: Array<Data | null>): ClientMessage {
        ...
        buf.fill(0);
        buf.writeUInt32LE(MapProjectWithPredicateCodec.RESPONSE_MESSAGE_TYPE >>> 0, 0);
