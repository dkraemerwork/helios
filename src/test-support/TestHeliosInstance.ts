...
export { MapProxy as SimpleMapProxy } from '@helios/map/impl/MapProxy';
...
export class TestHeliosInstance {
    ...
    private readonly multiMaps = new Map<string, MultiMapImpl<unknown, unknown>>();
    ...
    getMap<K, V>(name: string): IMap<K, V> {
        ...
        if (!proxy) {
            ...
            proxy = new MapProxy<unknown, unknown>(name, store, this.nodeEngine, this._mapContainerService);
    ...
    shutdown(): void {
        this.running = false;
        for (const topic of this.topics.values()) topic.destroy();
        this.maps.clear();
        this.queues.clear();
        this.lists.clear();
        this.sets.clear();
