...
import { ClusterJoinManager, type JoinTransport } from '@helios/internal/cluster/impl/ClusterJoinManager';
import { ClusterHeartbeatManager, type HeartbeatConfig } from '@helios/internal/cluster/impl/ClusterHeartbeatManager';
...
export class TestClusterNode {
    ...
    get clusterService(): ClusterServiceImpl { return this._clusterService; }
    get joinManager(): ClusterJoinManager { return this._joinManager; }
    get heartbeatManager(): ClusterHeartbeatManager { return this._heartbeatManager; }
    ...
    start(port?: number): void {
        ...
        this._heartbeatManager = new ClusterHeartbeatManager(this._clusterService, heartbeatConfig);
    ...
    private _handleJoinRequest(msg: ClusterMessage & { type: 'JOIN_REQUEST' }): void {
        if (!this._clusterService.isMaster()) return;

        const joinerAddress = new Address(msg.joinerAddress.host, msg.joinerAddress.port);
        const joinerMember = new MemberImpl.Builder(joinerAddress)
            .uuid(msg.joinerUuid)
            .version(new MemberVersion(msg.joinerVersion.major, msg.joinerVersion.minor, msg.joinerVersion.patch))
            .build();

        const result = this._joinManager.handleJoinRequest(
            joinerMember, msg.clusterName, msg.partitionCount,
        );
        ...
        if (result.accepted) {
            this._joinManager.startJoin([joinerMember]).then(() => {
                ...
                const wireMembers = members.map(m => ({
                    address: { host: m.getAddress().getHost(), port: m.getAddress().getPort() },
                    uuid: m.getUuid(),
                    attributes: Object.fromEntries(m.getAttributes()),
                    liteMember: m.isLiteMember(),
                    version: { major: m.getVersion().getMajor(), minor: m.getVersion().getMinor(), patch: m.getVersion().getPatch() },
                ...
                this.transport.send(msg.joinerUuid, {
                    ...
                    members: wireMembers,
                    masterAddress: { host: masterAddr.getHost(), port: masterAddr.getPort() },
                    clusterId: this._clusterService.getClusterId()!,
                });

                for (const m of members) {
                    if (m.getUuid() === this.nodeId || m.getUuid() === msg.joinerUuid) continue;
    ...
    private _handleFinalizeJoin(msg: ClusterMessage & { type: 'FINALIZE_JOIN' }): void {
        const masterAddress = new Address(msg.masterAddress.host, msg.masterAddress.port);
        this._clusterService.setMasterAddress(masterAddress);

        const members = msg.members.map(wm => {
            const isLocal = wm.uuid === this.nodeId;
            return new MemberImpl.Builder(new Address(wm.address.host, wm.address.port))
                .uuid(wm.uuid)
                .version(new MemberVersion(wm.version.major, wm.version.minor, wm.version.patch))
                .localMember(isLocal)
    ...
    private _handleMembersUpdate(msg: ClusterMessage & { type: 'MEMBERS_UPDATE' }): void {
        const members = msg.members.map(wm => {
            const isLocal = wm.uuid === this.nodeId;
            return new MemberImpl.Builder(new Address(wm.address.host, wm.address.port))
                .uuid(wm.uuid)
