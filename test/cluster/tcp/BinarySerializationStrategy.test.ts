...
describe('BinarySerializationStrategy', () => {
    it('round-trips the full current transport surface', () => {
        ...
        const messages: ClusterMessage[] = [
            ...
            operation,
            { type: 'OPERATION_RESPONSE', callId: 9, payload: { taskUuid: 't', status: 'success', originMemberUuid: 'node-a', resultData: sampleData(5), errorName: null, errorMessage: null } satisfies ExecutorOperationResult, error: null },
            backup,
            { type: 'BACKUP_ACK', callId: 9 },
            { type: 'RECOVERY_ANTI_ENTROPY', senderId: 'node-a', partitionId: 1, replicaIndex: 1, primaryVersions: ['1', '2'], namespaceVersions: { ns: ['3', '4'] } },
            { type: 'RECOVERY_SYNC_REQUEST', requesterId: 'node-b', partitionId: 1, replicaIndex: 1, dirtyNamespaces: ['ns'] },
            { type: 'RECOVERY_SYNC_RESPONSE', partitionId: 1, replicaIndex: 1, versions: ['1', '2'], namespaceVersions: { ns: ['3'] }, namespaceStates: [{ namespace: 'ns', estimatedSizeBytes: 12, entries: [[encoded, encoded]] }] },
