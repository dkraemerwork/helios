...
class StubHeliosInstance implements HeliosInstance {
    ...
    addMap(name: string): void {
        ...
        this.maps.set(name, { name, get: k => data.get(k), set: (k, v) => { data.set(k, v); } });
    }
    getMap<K, V>(name: string): IMap<K, V> { return this.maps.get(name) as unknown as IMap<K, V>; }
...
function makeMockTxContext(): TransactionContext & { store: Map<unknown, unknown>; commitCount: number; rollbackCount: number } {
...
function makeTxFactory(ctx: ReturnType<typeof makeMockTxContext>): TransactionContextFactory {
    return { create: () => ctx };
}
...
describe('HeliosModule + HeliosObjectExtractionModule + HeliosCacheModule — full context', () => {
    ...
    it('extracted IMap and CACHE_MANAGER work independently in full context', async () => {
        ...
        module = await Test.createTestingModule({
            imports: [
                HeliosModule.forRoot(hz),
                HeliosObjectExtractionModule.forRoot({ maps: { [MAP_TOKEN]: 'orders' } }),
                HeliosCacheModule.register(),
            ],
        }).compile();
    ...
    it('forRootAsync combined with object extraction and cache all work', async () => {
        ...
        module = await Test.createTestingModule({
            imports: [
                ...
                HeliosObjectExtractionModule.forRoot({ maps: { ASYNC_MAP: 'asyncMap' } }),
                HeliosCacheModule.register(),
            ],
        }).compile();
