...
describe('HeliosObjectExtractionModule.forRoot() — module metadata', () => {
    ...
    it('forRoot() with one map entry produces one provider', () => {
        const dm = HeliosObjectExtractionModule.forRoot({ maps: { MAP_TOKEN: 'myMap' } });
...
describe('HeliosObjectExtractionModule — NestJS DI integration', () => {
    ...
    it('registered map token resolves the IMap from HeliosInstance', async () => {
        ...
        module = await Test.createTestingModule({
            imports: [
                ...
                HeliosObjectExtractionModule.forRoot({ maps: { [MAP_TOKEN]: 'myMap' } }),
            ],
        }).compile();
    ...
    it('@Injectable service can inject extracted map via token', async () => {
        ...
        class OrderService {
            constructor(
                @Inject(ORDER_MAP_TOKEN) private readonly orderMap: StubMap<string, string>,
            ) {}
        ...
        module = await Test.createTestingModule({
            imports: [
                HeliosModule.forRoot(instance),
                HeliosObjectExtractionModule.forRoot({ maps: { [ORDER_MAP_TOKEN]: 'orderMap' } }),
            ],
    ...
    it('extracted map can be operated on (put + get)', async () => {
        ...
        module = await Test.createTestingModule({
            imports: [
                HeliosModule.forRoot(instance),
                HeliosObjectExtractionModule.forRoot({ maps: { [MAP_TOKEN]: 'dataMap' } }),
            ],
        }).compile();
