...
import { HeliosModule, HeliosInstanceFactory } from '@helios/nestjs/HeliosModule';
...
describe('HeliosModule.forRootAsync — useClass', () => {
        ...
        module = await Test.createTestingModule({
            imports: [
                HeliosModule.forRootAsync({
                    ...
                    extraProviders: [
                        { provide: NAME_TOKEN, useValue: 'injected-name' },
                    ],
                }),
            ],
        }).compile();
    ...
    it('useClass: supports async createHeliosInstance()', async () => {
        ...
        module = await Test.createTestingModule({
            imports: [
                HeliosModule.forRootAsync({ useClass: AsyncFactory }),
            ],
        }).compile();
    ...
    it('useClass: DynamicModule has module set to HeliosModule', () => {
        ...
        class DummyFactory implements HeliosInstanceFactory {
            createHeliosInstance(): HeliosInstance { return null!; }
        }
...
describe('HeliosModule.forRootAsync — imports field', () => {
    ...
    it('imports: modules imported in forRootAsync provide deps to factory', async () => {
        ...
        @Injectable()
        class ConfigService {
            getInstanceName(): string { return 'from-imports'; }
        }

        @Module({ providers: [{ provide: CONFIG_SERVICE_TOKEN, useClass: ConfigService }], exports: [CONFIG_SERVICE_TOKEN] })
        class ConfigModule {}
