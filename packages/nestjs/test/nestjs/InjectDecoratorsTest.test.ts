...
import { HeliosObjectExtractionModule } from '@helios/nestjs/HeliosObjectExtractionModule';
...
} from '@helios/nestjs/decorators';
import type { HeliosInstance } from '@helios/core/core/HeliosInstance';
import type { IMap } from '@helios/core/map/IMap';
import type { IQueue } from '@helios/core/collection/IQueue';
import type { IList } from '@helios/core/collection/IList';
import type { ISet } from '@helios/core/collection/ISet';
import type { ITopic } from '@helios/core/topic/ITopic';
import type { MultiMap } from '@helios/core/multimap/MultiMap';
import type { ReplicatedMap } from '@helios/core/replicatedmap/ReplicatedMap';
import type { DistributedObject } from '@helios/core/core/DistributedObject';
...
import type { HeliosConfig } from '@helios/core/config/HeliosConfig';
...
class StubHeliosInstance implements HeliosInstance {
    ...
    getMap<K, V>(name: string): IMap<K, V> {
        ...
        return { name, get: (k: K) => m.get(k) as V, set: (k: K, v: V) => { m.set(k, v); } } as unknown as IMap<K, V>;
    }
    getQueue<E>(name: string): IQueue<E> {
        if (!this._queues.has(name)) this._queues.set(name, []);
        const q = this._queues.get(name)!;
        return { name, add: (e: E) => { q.push(e); }, peek: () => q[0] as E } as unknown as IQueue<E>;
    }
    ...
    getList<E>(name: string): IList<E> {
        ...
        return { name, add: (e: E) => { l.push(e); }, size: () => l.length } as unknown as IList<E>;
    }
    getSet<E>(name: string): ISet<E> {
        if (!this._sets.has(name)) this._sets.set(name, new Set());
        const s = this._sets.get(name)!;
        return { name, add: (e: E) => { s.add(e); }, has: (e: E) => s.has(e) } as unknown as ISet<E>;
    }
...
describe('Token helper functions', () => {
    ...
    it('getReplicatedMapToken returns prefixed token', () => {
        expect(getReplicatedMapToken('config')).toBe('HELIOS_REPLICATED_MAP_config');
    });
...
describe('@InjectMap() decorator — DI integration', () => {
    ...
    it('@InjectMap(name) resolves an IMap via HeliosObjectExtractionModule', async () => {
        ...
        class UserService {
            constructor(
                @InjectMap('users') readonly users: IMap<string, string>,
            ) {}
        }
    ...
    it('@InjectQueue(name) resolves an IQueue via HeliosObjectExtractionModule', async () => {
        ...
        class TaskService {
            constructor(
                @InjectQueue('tasks') readonly tasks: IQueue<string>,
            ) {}
        }
    ...
    it('@InjectTopic(name) resolves an ITopic via HeliosObjectExtractionModule', async () => {
        ...
        class EventBus {
            constructor(
                @InjectTopic('events') readonly events: ITopic<string>,
            ) {}
        }
    ...
    it('@InjectList(name) resolves an IList via HeliosObjectExtractionModule', async () => {
        ...
        class ListService {
            constructor(
                @InjectList('items') readonly items: IList<string>,
            ) {}
        }
    ...
    it('@InjectSet(name) resolves an ISet via HeliosObjectExtractionModule', async () => {
        ...
        class TagService {
            constructor(
                @InjectSet('tags') readonly tags: ISet<string>,
            ) {}
        }
    ...
    it('@InjectMultiMap(name) resolves a MultiMap via HeliosObjectExtractionModule', async () => {
        ...
        class RelationService {
            constructor(
                @InjectMultiMap('relations') readonly relations: MultiMap<string, string>,
            ) {}
        }
