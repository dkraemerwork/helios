 ...
 *
 * Validates that @helios/blitz meets the 80%+ parity contract with Hazelcast Jet.
...
function makeIMapStub<K, V>(name = 'test-map') {
    ...
    return {
        ...
        size: () => data.size,
        data,
    } as unknown as import('@helios/map/IMap').IMap<K, V> & {
        data: Map<K, V>;
        size(): number;
    };
}
...
describe('Feature parity gate — @helios/blitz v1.0 requirements', () => {
    ...
    it('NestJS module for @helios/blitz is importable', async () => {
