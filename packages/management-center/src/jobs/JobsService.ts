    ...
    await this.queue.enqueue(async () => {
      ...
      const stmts = snapshots.map((s) => ({
        sql: `INSERT INTO job_snapshots (
          ...
          light_job, supports_cancel, supports_restart,
        ...
        args: [
          ...
          s.completionTime,
          s.lightJob ? 1 : 0,
          s.supportsCancel ? 1 : 0,
          s.supportsRestart ? 1 : 0,
          s.metricsJson,
          s.verticesJson,
          s.edgesJson,
        ] as InValue[],
...
function rowToJobSnapshot(row: Record<string, unknown>): JobSnapshot {
  return {
    ...
    lightJob: Number(row['light_job'] ?? 0) === 1,
    supportsCancel: Number(row['supports_cancel'] ?? 1) === 1,
    supportsRestart: Number(row['supports_restart'] ?? 1) === 1,
