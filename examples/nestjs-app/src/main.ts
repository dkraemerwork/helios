...
import { MapStoreConfig } from '@helios/core/config/MapStoreConfig';
import { NearCacheConfig } from '@helios/core/config/NearCacheConfig';
import { MongoMapStore } from '@helios/mongodb';
import { S3MapStore } from '@helios/s3';
import { TursoMapStore } from '@helios/turso';
...
console.log('  Sessions persisted to ./data/sessions.db (survives restarts).\n');
...
try {
    ...
    console.log('  Start MongoDB and re-run, or set MONGO_URI=mongodb://host:port');
}
...
console.log('  Requires: MinIO/LocalStack on localhost:9000 (or S3_ENDPOINT env)\n');
...
try {
    ...
    console.log('  Start MinIO/LocalStack and re-run, or set S3_ENDPOINT=http://host:port');
}
