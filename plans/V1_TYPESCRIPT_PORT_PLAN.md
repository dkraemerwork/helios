...
This master plan must always be interpreted against the repo as it exists now.

- The repo uses `@zenystx/helios-core/*`, not `@helios/*`.
- Example/demo code lives under `examples/native-app/` and `examples/nestjs-app/`, not `app/`.
- Test support lives under `src/test-support/`, not a root `test-support/` package.
- Config loading is currently JSON/YAML via `src/config/ConfigLoader.ts`; XML support is not yet
  present and must not be assumed complete unless explicitly implemented.
- Every user-facing phase must include a public-surface gate: `src/index.ts` exports,
  `package.json` exports, README/examples, config loader wiring, and negative tests for any
  deferred-but-public API stubs.
- Existing deferred stubs such as `getSql()`, `getCPSubsystem()`, and
  `getScheduledExecutorService()` must stay explicitly documented and tested until replaced.
...
| `jet/` (520 files) | Java DAG engine — NOT ported line-by-line. **Replaced by `@zenystx/helios-blitz` (Phase 10)** using NATS JetStream. See Phase 10 blocks 10.0–10.10. |
...
├── package.json                          # @zenystx/helios-core, Bun, TS beta
...
│
├── packages/
│   └── nestjs/                           # @zenystx/helios-nestjs (Phase 9)
│       ├── package.json                  # @zenystx/helios-nestjs, deps: @zenystx/helios-core + NestJS 11
│       ├── tsconfig.json                 # ES2025, paths: @zenystx/helios-core/* → ../../src/*
...
@zenystx/helios-core (repo root) + @zenystx/helios-nestjs (packages/nestjs/) Bun workspace
...
@zenystx/helios-core IMap/ITopic + nats npm package
        ...
        └─► 10.1 Pipeline/DAG builder (Vertex, Edge, submit/cancel)
              ...
              └─► 10.3 stream operators (parallel with 10.2)
                    ...
                    └─► 10.7 fault tolerance (AckPolicy, retry, DL, checkpoint)
                          ...
                          └─► 10.9 NestJS module (@zenystx/helios-blitz)
...
[Phase 17 — depends on Phase 16 (Multi-Node Resilience) + `@zenystx/scatterjs`]
OperationService + PartitionService + ClusterService + `@zenystx/scatterjs` `scatter.pool()`
  └─► 17.0 runtime foundation + `@zenystx/scatterjs` package dependency → 17.1 ExecutorConfig
...
> For the full release-grade client program, including public API, connection/auth lifecycle,
> proxy manager, remote distributed-object proxies, listener recovery, failover/metrics, and
> acceptance gates for a separate Bun app connecting to a Helios cluster, see
...
## Phase 9 — `@zenystx/helios-nestjs` Package Extraction + Modern NestJS Library Patterns

Goal: Extract NestJS integration into a separate `@zenystx/helios-nestjs` package, then modernize it
to state-of-the-art NestJS 11 library patterns. The core `@zenystx/helios-core` package must have
...
Rename root package: `"name": "helios"` → `"name": "@zenystx/helios-core"`.
Add wildcard subpath export: `"./*": { "import": "./dist/src/*.js", "types": "./dist/src/*.d.ts" }`.
Remove all NestJS deps from `@zenystx/helios-core`.
...
`test/nestjs/` into `packages/nestjs/` as `@zenystx/helios-nestjs`. Copy, transform
...
- Intra-NestJS: `@zenystx/helios-nestjs/X` → `./X` (relative within package)
- Core types: `@zenystx/helios-core/HeliosInstance` → `@zenystx/helios-core/core/HeliosInstance`
- Core modules: `@helios/transaction/X` → `@zenystx/helios-core/transaction/X`
- Core SPI: `@helios/spi/impl/X` → `@zenystx/helios-core/spi/impl/X`

```
packages/nestjs/
├── package.json            # @zenystx/helios-nestjs, deps: @zenystx/helios-core + NestJS
├── tsconfig.json           # paths: @zenystx/helios-core/* → ../../src/*
├── bunfig.toml             # preload: reflect-metadata
...
- [x] Rename root package to `@zenystx/helios-core`, remove NestJS deps, add `./*` subpath export
...
- [x] `git commit -m "refactor(nestjs): extract @zenystx/helios-nestjs package — 141 tests green"`
...
Finalize the `@zenystx/helios-nestjs` package for publication.
...
- [x] Verify `bun publish --dry-run` for both `@zenystx/helios-core` and `@zenystx/helios-nestjs`
- [x] GREEN
- [x] `git commit -m "feat(nestjs): @zenystx/helios-nestjs v1.0 — all tests green"`
...
**Phase 9 done gate**: `@zenystx/helios-nestjs` is a standalone, publishable NestJS library with:
...
Goal: deliver a `@zenystx/helios-blitz` package that provides ~80%+ feature parity with Hazelcast
...
```
packages/blitz/                              # @zenystx/helios-blitz
├── package.json                           # deps: @nats-io/transport-node, @nats-io/jetstream, @nats-io/kv, @zenystx/helios-core
...
│   └── nestjs/                            # Block 10.9 — exported via @zenystx/helios-blitz/nestjs subpath
...
`package.json` exports field includes a `@zenystx/helios-blitz/nestjs` subpath:
...
use `@zenystx/helios-blitz` for pure stream processing without NestJS do not need `@nestjs/common`
...
All `@zenystx/helios-blitz` errors extend `BlitzError`. Operators signal recoverable failures via
...
```
packages/blitz/
├── package.json            # @zenystx/helios-blitz
...
- [x] Create `packages/blitz/` directory with `package.json` (`@zenystx/helios-blitz`, deps: `@nats-io/transport-node`, `@nats-io/jetstream`, `@nats-io/kv`, `@zenystx/helios-core`; peerDeps optional: `@nestjs/common@^11`, `@nestjs/core@^11`; exports: `"."` + `"@zenystx/helios-blitz/nestjs"`), `tsconfig.json`, `bunfig.toml`, `src/index.ts` — root `package.json` workspace entry already configured
...
`JsonCodec<T>()` is from `@zenystx/helios-blitz/codec` — it decodes the raw `Uint8Array` NATS payload
...
`@zenystx/helios-blitz` introduces a `BlitzCodec<T>` interface so every source/sink has an explicit
wire format:
...
- [x] Helios sources/sinks: wire to `@zenystx/helios-core` IMap / ITopic interfaces
...
delegate to the corresponding `@zenystx/helios-core` implementations and add `combine()`:

- `CountAggregator` wraps `@zenystx/helios-core/aggregation/CountAggregator` — `combine(a, b) = a + b`
- `SumAggregator` wraps `@zenystx/helios-core/aggregation/SumAggregator` — `combine(a, b) = a + b`
...
No duplicate business logic. `@zenystx/helios-core` remains the authoritative implementation.
...
- [x] Implement blitz aggregate wrappers that delegate to `@zenystx/helios-core` aggregators and add `combine()` — no business logic duplication
...
- [x] Implement all 6 concrete aggregators as thin wrappers over `@zenystx/helios-core/aggregation/` implementations (reuse core logic; add only `combine()`)
...
### Block 10.9 — NestJS integration (`@zenystx/helios-blitz` module) (~25 tests)
...
- `@nestjs/common@^11` and `@nestjs/core@^11` are declared as **optional peer dependencies**
  ...
  submodule. Applications that use `@zenystx/helios-blitz` for pure stream processing without NestJS
  do not need them.
- `@zenystx/helios-blitz` must **NOT** import from `src/nestjs/` in its main barrel `src/index.ts`.
  The NestJS submodule is exported via a separate subpath: `@zenystx/helios-blitz/nestjs`.
- Consumers import NestJS integration as: `import { HeliosBlitzModule } from '@zenystx/helios-blitz/nestjs'`

**TODO — Block 10.9**:
- [x] Add `@nestjs/common@^11` and `@nestjs/core@^11` as optional peer dependencies in `packages/blitz/package.json`
- [x] Export `src/nestjs/` via `@zenystx/helios-blitz/nestjs` subpath export in `packages/blitz/package.json` — NOT from main barrel `src/index.ts`
...
- [x] GREEN
- [x] `git commit -m "feat(blitz): @zenystx/helios-blitz NestJS module integration — 25 tests green"`
...
This block validates that the assembled `@zenystx/helios-blitz` package meets the 80%+ parity
...
**Feature parity gate** (blocks release of `@zenystx/helios-blitz` v1.0):
...
- [x] `git commit -m "test(blitz): Block 10.10 — e2e acceptance + feature parity gate — @zenystx/helios-blitz v1.0 — 33 tests green"`

---

**Phase 10 done gate**: `@zenystx/helios-blitz` v1.0 is a standalone, publishable Bun/TypeScript
...
- `@zenystx/helios-blitz` NestJS module with `forRoot`/`forRootAsync` + `@InjectBlitz()` decorator
...
Goal: promote the REST API from a demo-app concern into `@zenystx/helios-core` as a proper
...
**Phase 11 done gate**: Built-in REST API is a first-class feature of `@zenystx/helios-core`:
...
> **No longer deferred** — implemented in **Phase 10** as `@zenystx/helios-blitz`, a NATS JetStream-backed
...
### Phase 9 — `@zenystx/helios-nestjs` Package Extraction + Modern NestJS Library Patterns
- [x] **Block 9.0** — Package extraction: Bun workspace, @zenystx/helios-core rename, @zenystx/helios-nestjs extraction — 168 tests ✅ (2157 core + 168 nestjs)
...
- [x] **Phase 9 checkpoint**: `@zenystx/helios-nestjs` v1.0 — state-of-the-art NestJS library (~80 new tests) ✅
...
- [x] **Phase 10 checkpoint**: `@zenystx/helios-blitz` v1.0 — NATS-backed stream & batch engine, ~80% Hazelcast Jet parity, 328 tests green ✅
...
- [x] **Phase 11 checkpoint**: REST API is a first-class `@zenystx/helios-core` feature — K8s probes, data access, cluster ops via `curl` — ~56 tests green ✅
...
- [x] **Block 12.B** — `packages/s3/` (`@zenystx/helios-s3`): `S3MapStore` + `S3Config` + `S3MapStore.factory(baseConfig)` (factory scopes prefix by map name), mock-S3-client tests, factory tests (2), workspace wiring — 14 tests ✅
- [x] **Block 12.C** — `packages/mongodb/` (`@zenystx/helios-mongodb`): `MongoMapStore` + `MongoConfig` + `MongoMapStore.factory(baseConfig)` (factory scopes collection by map name), mock-collection tests, factory tests (2), workspace wiring — 15 tests ✅
- [x] **Block 12.D** — `packages/turso/` (`@zenystx/helios-turso`): `TursoMapStore` + `TursoConfig` + `TursoMapStore.factory(baseConfig)` (factory scopes tableName by map name), real in-memory SQLite tests (`:memory:`), factory tests (2), workspace wiring — 18 tests ✅
...
> Goal: Embed a NATS JetStream server natively inside `@zenystx/helios-blitz` so that users never need
...
> **Depends on:** Phase 16 (OperationService routing, partition system, backup replication) + `@zenystx/scatterjs`
...
- [x] **Block 17.0** — Executor runtime foundation + `@zenystx/scatterjs` package dependency (close remote OperationService gap, expose cluster/partition routing surfaces, add `@zenystx/scatterjs` dependency) — 13 tests ✅
...
- [x] **Block 17.9C** — Harden task registration for worker materialization (worker-safe metadata, fingerprint inputs, inline-vs-distributed enforcement) — 9 tests ✅
...
> hosts one local embedded NATS node; the current Helios master acts as the Blitz bootstrap
> authority for topology + seed routing; starting a second node should automatically form a
> shared NATS/JetStream cluster without a manual master env flag. Raw Blitz owns only the local
> embedded node primitive; Helios owns the distributed control plane, protocol, lifecycle wiring,
> and replication reconciliation.
...
- [ ] **Block 18.2** — Helios Blitz config + protocol + topology service (`HeliosConfig` Blitz runtime section, topology models, coordinator service, `BLITZ_*` cluster messages with `requestId`/retry metadata, current-master snapshot authority using `memberListVersion`, mandatory re-registration after master change) — ~18 tests
